...
export function ReciboDocument({ company, quote, items }: {
  ...
  return (
    <article className="mx-auto max-w-3xl space-y-6 p-4 text-slate-800 print:p-0">
      ...
      {/* Tabela de serviços */}
      <section>
        <div className="grid grid-cols-[1fr_3rem_8rem] gap-3 border-b pb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          <span>Descrição</span><span className="text-right">Qtd</span><span className="text-right">Total</span>
        </div>
        {items.map((it, i) => (
          <div key={i} className="grid grid-cols-[1fr_3rem_8rem] gap-3 border-b py-3 text-sm">
