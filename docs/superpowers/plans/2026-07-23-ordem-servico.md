...
begin
  ...
  loop
    ...
    -- clona antes do status bloquear: o trigger woc_closed_guard recusa
    -- lançamento em OS concluída, então as arquivadas voltam a 'planejada'
    -- durante o clone e são reencerradas em seguida.
    update work_orders set status = 'planejada' where id = v_id;
